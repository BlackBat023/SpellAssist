CITATION.cff
LICENSE.md
MANIFEST.in
README.md
pypi_to_import
requirements.txt
setup.cfg
setup.py
SpiNNaker_PACMAN.egg-info/PKG-INFO
SpiNNaker_PACMAN.egg-info/SOURCES.txt
SpiNNaker_PACMAN.egg-info/dependency_links.txt
SpiNNaker_PACMAN.egg-info/requires.txt
SpiNNaker_PACMAN.egg-info/top_level.txt
pacman/__init__.py
pacman/_version.py
pacman/exceptions.py
pacman/executor/__init__.py
pacman/executor/algorithm_metadata_xml_reader.py
pacman/executor/injection_decorator.py
pacman/executor/pacman_algorithm_executor.py
pacman/executor/token_states.py
pacman/executor/algorithm_classes/__init__.py
pacman/executor/algorithm_classes/abstract_algorithm.py
pacman/executor/algorithm_classes/abstract_python_algorithm.py
pacman/executor/algorithm_classes/external_algorithm.py
pacman/executor/algorithm_classes/python_class_algorithm.py
pacman/executor/algorithm_classes/python_function_algorithm.py
pacman/executor/algorithm_decorators/__init__.py
pacman/executor/algorithm_decorators/abstract_input.py
pacman/executor/algorithm_decorators/algorithm_decorator.py
pacman/executor/algorithm_decorators/all_of_input.py
pacman/executor/algorithm_decorators/one_of_input.py
pacman/executor/algorithm_decorators/output.py
pacman/executor/algorithm_decorators/single_input.py
pacman/executor/algorithm_decorators/token.py
pacman/model/__init__.py
pacman/model/constraints/__init__.py
pacman/model/constraints/abstract_constraint.py
pacman/model/constraints/key_allocator_constraints/__init__.py
pacman/model/constraints/key_allocator_constraints/abstract_key_allocator_constraint.py
pacman/model/constraints/key_allocator_constraints/contiguous_key_range_constraint.py
pacman/model/constraints/key_allocator_constraints/fixed_key_and_mask_constraint.py
pacman/model/constraints/key_allocator_constraints/fixed_key_field_constraint.py
pacman/model/constraints/key_allocator_constraints/fixed_mask_constraint.py
pacman/model/constraints/key_allocator_constraints/share_key_constraint.py
pacman/model/constraints/partitioner_constraints/__init__.py
pacman/model/constraints/partitioner_constraints/abstract_partitioner_constraint.py
pacman/model/constraints/partitioner_constraints/fixed_vertex_atoms_constraint.py
pacman/model/constraints/partitioner_constraints/max_vertex_atoms_constraint.py
pacman/model/constraints/partitioner_constraints/same_atoms_as_vertex_constraint.py
pacman/model/constraints/placer_constraints/__init__.py
pacman/model/constraints/placer_constraints/abstract_placer_constraint.py
pacman/model/constraints/placer_constraints/board_constraint.py
pacman/model/constraints/placer_constraints/chip_and_core_constraint.py
pacman/model/constraints/placer_constraints/radial_placement_from_chip_constraint.py
pacman/model/constraints/placer_constraints/same_chip_as_constraint.py
pacman/model/graphs/__init__.py
pacman/model/graphs/abstract_edge.py
pacman/model/graphs/abstract_edge_partition.py
pacman/model/graphs/abstract_fpga.py
pacman/model/graphs/abstract_multiple_partition.py
pacman/model/graphs/abstract_single_source_partition.py
pacman/model/graphs/abstract_spinnaker_link.py
pacman/model/graphs/abstract_supports_sdram_edges.py
pacman/model/graphs/abstract_vertex.py
pacman/model/graphs/abstract_virtual.py
pacman/model/graphs/graph.py
pacman/model/graphs/application/__init__.py
pacman/model/graphs/application/application_edge.py
pacman/model/graphs/application/application_edge_partition.py
pacman/model/graphs/application/application_fpga_vertex.py
pacman/model/graphs/application/application_graph.py
pacman/model/graphs/application/application_spinnaker_link_vertex.py
pacman/model/graphs/application/application_vertex.py
pacman/model/graphs/application/abstract/__init__.py
pacman/model/graphs/application/abstract/abstract_one_app_one_machine_vertex.py
pacman/model/graphs/common/__init__.py
pacman/model/graphs/common/constrained_object.py
pacman/model/graphs/common/edge_traffic_type.py
pacman/model/graphs/common/slice.py
pacman/model/graphs/machine/__init__.py
pacman/model/graphs/machine/abstract_machine_edge_partition.py
pacman/model/graphs/machine/abstract_sdram_partition.py
pacman/model/graphs/machine/constant_sdram_machine_partition.py
pacman/model/graphs/machine/destination_segmented_sdram_machine_partition.py
pacman/model/graphs/machine/fixed_route_edge_partition.py
pacman/model/graphs/machine/machine_edge.py
pacman/model/graphs/machine/machine_fpga_vertex.py
pacman/model/graphs/machine/machine_graph.py
pacman/model/graphs/machine/machine_sdram_edge.py
pacman/model/graphs/machine/machine_spinnaker_link_vertex.py
pacman/model/graphs/machine/machine_vertex.py
pacman/model/graphs/machine/multicast_edge_partition.py
pacman/model/graphs/machine/simple_machine_vertex.py
pacman/model/graphs/machine/source_segmented_sdram_machine_partition.py
pacman/model/partitioner_interfaces/__init__.py
pacman/model/partitioner_interfaces/abstract_slices_connect.py
pacman/model/partitioner_interfaces/abstract_splitter_partitioner.py
pacman/model/partitioner_interfaces/legacy_partitioner_api.py
pacman/model/partitioner_splitters/__init__.py
pacman/model/partitioner_splitters/splitter_one_app_one_machine.py
pacman/model/partitioner_splitters/splitter_one_to_one_legacy.py
pacman/model/partitioner_splitters/splitter_slice_legacy.py
pacman/model/partitioner_splitters/abstract_splitters/__init__.py
pacman/model/partitioner_splitters/abstract_splitters/abstract_dependent_splitter.py
pacman/model/partitioner_splitters/abstract_splitters/abstract_splitter_common.py
pacman/model/partitioner_splitters/abstract_splitters/abstract_splitter_slice.py
pacman/model/placements/__init__.py
pacman/model/placements/placement.py
pacman/model/placements/placements.py
pacman/model/resources/__init__.py
pacman/model/resources/abstract_sdram.py
pacman/model/resources/constant_sdram.py
pacman/model/resources/core_resource.py
pacman/model/resources/cpu_cycles_per_tick_resource.py
pacman/model/resources/dtcm_resource.py
pacman/model/resources/element_free_space.py
pacman/model/resources/iptag_resource.py
pacman/model/resources/multi_region_sdram.py
pacman/model/resources/pre_allocated_resource_container.py
pacman/model/resources/resource_container.py
pacman/model/resources/reverse_iptag_resource.py
pacman/model/resources/specific_board_iptag_resource.py
pacman/model/resources/specific_board_reverse_iptag_resource.py
pacman/model/resources/specific_chip_sdram_resource.py
pacman/model/resources/specific_core_resource.py
pacman/model/resources/variable_sdram.py
pacman/model/routing_info/__init__.py
pacman/model/routing_info/abstract_machine_partition_n_keys_map.py
pacman/model/routing_info/base_key_and_mask.py
pacman/model/routing_info/dict_based_machine_partition_n_keys_map.py
pacman/model/routing_info/partition_routing_info.py
pacman/model/routing_info/routing_info.py
pacman/model/routing_table_by_partition/__init__.py
pacman/model/routing_table_by_partition/multicast_routing_table_by_partition.py
pacman/model/routing_table_by_partition/multicast_routing_table_by_partition_entry.py
pacman/model/routing_tables/__init__.py
pacman/model/routing_tables/abstract_multicast_routing_table.py
pacman/model/routing_tables/compressed_multicast_routing_table.py
pacman/model/routing_tables/multicast_routing_tables.py
pacman/model/routing_tables/uncompressed_multicast_routing_table.py
pacman/model/tags/__init__.py
pacman/model/tags/tags.py
pacman/operations/__init__.py
pacman/operations/algorithms_metadata.xml
pacman/operations/algorithms_metadata_schema.xsd
pacman/operations/algorithm_reports/__init__.py
pacman/operations/algorithm_reports/network_specification.py
pacman/operations/algorithm_reports/reports.py
pacman/operations/algorithm_reports/reports_metadata.xml
pacman/operations/algorithm_reports/router_collision_potential_report.py
pacman/operations/algorithm_reports/router_summary.py
pacman/operations/algorithm_reports/write_json_machine.py
pacman/operations/algorithm_reports/write_json_machine_graph.py
pacman/operations/algorithm_reports/write_json_partition_n_keys_map.py
pacman/operations/algorithm_reports/write_json_placements.py
pacman/operations/algorithm_reports/write_json_routing_tables.py
pacman/operations/chip_id_allocator_algorithms/__init__.py
pacman/operations/chip_id_allocator_algorithms/malloc_based_chip_id_allocator.py
pacman/operations/fixed_route_router/__init__.py
pacman/operations/fixed_route_router/fixed_route_router.py
pacman/operations/multi_cast_router_check_functionality/__init__.py
pacman/operations/multi_cast_router_check_functionality/valid_routes_checker.py
pacman/operations/partition_algorithms/__init__.py
pacman/operations/partition_algorithms/splitter_partitioner.py
pacman/operations/placer_algorithms/__init__.py
pacman/operations/placer_algorithms/connective_based_placer.py
pacman/operations/placer_algorithms/one_to_one_placer.py
pacman/operations/placer_algorithms/radial_placer.py
pacman/operations/placer_algorithms/spreader_placer.py
pacman/operations/rigged_algorithms/__init__.py
pacman/operations/rigged_algorithms/hilbert_placer.py
pacman/operations/rigged_algorithms/hilbert_state.py
pacman/operations/rigged_algorithms/isomorph_check.py
pacman/operations/rigged_algorithms/random_placer.py
pacman/operations/router_algorithms/__init__.py
pacman/operations/router_algorithms/basic_dijkstra_routing.py
pacman/operations/router_algorithms/ner_route.py
pacman/operations/router_algorithms/routing_tree.py
pacman/operations/router_compressors/__init__.py
pacman/operations/router_compressors/abstract_compressor.py
pacman/operations/router_compressors/basic_route_merger.py
pacman/operations/router_compressors/checked_unordered_pair_compressor.py
pacman/operations/router_compressors/entry.py
pacman/operations/router_compressors/malloc_based_route_merger.py
pacman/operations/router_compressors/pair_compressor.py
pacman/operations/router_compressors/routing_compression_checker.py
pacman/operations/router_compressors/unordered_pair_compressor.py
pacman/operations/router_compressors/ordered_covering_router_compressor/__init__.py
pacman/operations/router_compressors/ordered_covering_router_compressor/ordered_covering.py
pacman/operations/router_compressors/ordered_covering_router_compressor/ordered_covering_compressor.py
pacman/operations/router_compressors/ordered_covering_router_compressor/remove_default_routes.py
pacman/operations/router_compressors/ordered_covering_router_compressor/utils.py
pacman/operations/routing_info_allocator_algorithms/__init__.py
pacman/operations/routing_info_allocator_algorithms/basic_routing_info_allocator.py
pacman/operations/routing_info_allocator_algorithms/destination_based_key_allocator.py
pacman/operations/routing_info_allocator_algorithms/zoned_routing_info_allocator.py
pacman/operations/routing_info_allocator_algorithms/malloc_based_routing_allocator/__init__.py
pacman/operations/routing_info_allocator_algorithms/malloc_based_routing_allocator/compressible_malloc_based_routing_info_allocator.py
pacman/operations/routing_info_allocator_algorithms/malloc_based_routing_allocator/key_field_generator.py
pacman/operations/routing_info_allocator_algorithms/malloc_based_routing_allocator/malloc_based_routing_info_allocator.py
pacman/operations/routing_info_allocator_algorithms/malloc_based_routing_allocator/utils.py
pacman/operations/routing_table_generators/__init__.py
pacman/operations/routing_table_generators/basic_routing_table_generator.py
pacman/operations/routing_table_generators/zoned_routing_table_generator.py
pacman/operations/tag_allocator_algorithms/__init__.py
pacman/operations/tag_allocator_algorithms/basic_tag_allocator.py
pacman/utilities/__init__.py
pacman/utilities/constants.py
pacman/utilities/json_utils.py
pacman/utilities/utility_calls.py
pacman/utilities/vertex_sorter.py
pacman/utilities/algorithm_utilities/__init__.py
pacman/utilities/algorithm_utilities/element_allocator_algorithm.py
pacman/utilities/algorithm_utilities/machine_algorithm_utilities.py
pacman/utilities/algorithm_utilities/partition_algorithm_utilities.py
pacman/utilities/algorithm_utilities/placer_algorithm_utilities.py
pacman/utilities/algorithm_utilities/routing_info_allocator_utilities.py
pacman/utilities/file_format_schemas/__init__.py
pacman/utilities/file_format_schemas/machine.json
pacman/utilities/file_format_schemas/machine_graph.json
pacman/utilities/file_format_schemas/n_keys_map.json
pacman/utilities/file_format_schemas/placements.json
pacman/utilities/file_format_schemas/routing_tables.json
pacman/utilities/utility_objs/__init__.py
pacman/utilities/utility_objs/field.py
pacman/utilities/utility_objs/resource_tracker.py